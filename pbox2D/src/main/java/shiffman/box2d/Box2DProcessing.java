...
public class Box2DProcessing {
	...
	public Box2DProcessing(PApplet p, float sf) {
		parent = p;
		transX = parent.width / 2.0F;
		transY = parent.height / 2.0F;
		scaleFactor = sf;
	...
	public void createWorld(Vec2 gravity, boolean warmStarting, boolean continous) {
		world = new World(gravity, continous);
	...
	public Vec2 coordPixelsToWorld(float pixelX, float pixelY) {
		...
		float worldY = pixelY;
		if (yFlip == Y_FLIP_INDICATOR) worldY = PApplet.map(pixelY,parent.height,0.0f,0.0f,parent.height);
